...
- (void)xmppStreamDidDisconnect:(XMPPStream *)sender withError:(NSError *)error
{
    ...
    if ([self autoReconnect] && [self shouldReconnect] && [self multipleReachabilityChanges])
    {

    }
    else{
        
        //[[[UIApplication sharedApplication] delegate] performSelector:@selector(ReconnectInterNet)];
    }
}
...
static void XMPPReconnectReachabilityCallback(SCNetworkReachabilityRef target, SCNetworkReachabilityFlags flags, void *info)
{
	@autoreleasepool {
        ...
        {
            NSLog(@"Internet now Works and auto Connection");
            //[[[UIApplication sharedApplication] delegate] performSelector:@selector(ReconnectInterNet)];
        }
