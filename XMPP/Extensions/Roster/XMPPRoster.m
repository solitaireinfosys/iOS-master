...
- (void)deleteUser:(XMPPJID *)jid withNickname:(NSString *)optionalName{
    [self addUser:jid withNickname:optionalName groups:nil subscribeToPresence:NO];
}


- (void)addUser:(XMPPJID *)jid withNickname:(NSString *)optionalName{
    XMPPPresence *response = [XMPPPresence presenceWithType:@"subscribed" to:jid];
...
- (void)xmppStream:(XMPPStream *)sender didReceivePresence:(XMPPPresence *)presence
{
	...
	if ([[presence type] isEqualToString:@"subscribe"])
	{
		...
		else
		{
            ...
            [self subscribePresenceToUser:userJID];
            NSLog(@"%@",[presence from]);
            [self addUser:userJID withNickname:[NSString stringWithFormat:@"%@",[presence from]] ];
		}
	}
    else if ([[presence type] isEqualToString:@"unsubscribe"])
    {
        [[SSAddFriend shareInstance] deleteFriendWithJid:[[presence from] bare]];
        
        //update unread message count
        [[SSConnectionClasses shareInstance] updateUserUnreadMessageOfJID:[[presence from] bare]];
...
{
	// This method is invoked on the moduleQueue.
    NSLog(@"Disconnect in Xmpp Roster");
	XMPPLogTrace();
    [multicastDelegate xmppStreamDidDisconnect:sender withError:error];
    if([self autoClearAllUsersAndResources])
    {
        if ([error.description isEqualToString:@"Socket is not connected"])
        {
